		...
		private void DestroyProjectile()
		{
			if (explosionOnHit)
			{
				...
				foreach (var collider in colliders)
				{
					...
					if (collider.gameObject.TryGetComponent(out DestroyIt.Destructible dest))
					{
						Debug.Log("Event Çalıştı");
						Zombie zombie=collider.GetComponentInParent<Zombie>();
						dest.ApplyDamage(1000);
			
							if (collider.gameObject.tag == "Head")
							{
								collider.gameObject.GetComponentInParent<Zombie>().SetDead();
							}
							else if(collider.gameObject.tag == "Leg"&&zombie.isFall==false)
							{
								collider.gameObject.GetComponentInParent<Zombie>().SetFall();
							}
