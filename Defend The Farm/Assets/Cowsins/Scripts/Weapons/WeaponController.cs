...
namespace cowsins
{
	public class WeaponController : MonoBehaviour
	{
		...
		private void Hit(LayerMask layer, float damage, RaycastHit h, bool damageTarget)
		{
			...
			if (h.collider.gameObject.TryGetComponent(out DestroyIt.Destructible destructible))
			{
				destructible.ApplyDamage(weapon.damagePerBullet);
				destructible.gameObject.GetComponentInParent<Zombie>().DamageSpawn(hit.point,(int)weapon.damagePerBullet);
			...
			if (h.collider.gameObject.CompareTag("WereWolfHead"))
			{
				CowsinsUtilities.GatherDamageableParent(h.collider.transform).Damage(weapon.damagePerBullet, false);
				
				h.collider.gameObject.GetComponentInParent<IEnemies>().DamageSpawn(hit.point,(int)weapon.damagePerBullet);
			}
			if (h.collider.gameObject.tag!="WereWolfHead"&&h.collider.gameObject.tag!="Head"&&h.collider.GetComponent<DestroyIt.Destructible>()==null)
			{
				IEnemies enemies=h.collider.gameObject.GetComponentInParent<IEnemies>();
				if (enemies!=null)
				{
					enemies.NoDamageSpawn(hit.point);
				}
			...
			{
				h.collider.GetComponent<IDamageable>().Damage(weapon.damagePerBullet, false);
