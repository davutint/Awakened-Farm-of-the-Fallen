		...
		public void ApplyDamage(float amount)
		{
			...
			if (IsDestroyed)
			{
				...
				if(gameObject.CompareTag("Head"))
				{
					...
					gameObject.GetComponentInParent<Zombie>().SetDead();
				}
				else if(gameObject.CompareTag("Leg"))
				{
					gameObject.GetComponentInParent<Zombie>().SetFall();
					
				}
				gameObject.GetComponentInParent<Zombie>().Dismemberment(gameObject.transform.position);
			}
